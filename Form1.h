...
#pragma endregion
	...
	private: void ListDevices()																//вывод списка USB-устройств
	{
		...
		if (prevList.size() == 0)
		{
			listBox1->Items->Add("No USB devices connected.");								//åñëè íåò ïîäêëþ÷åíûõ USB-óñòðîéñòâ
		}
		for (int i = 0; i < prevList.size(); i++)
		{
			if (!(Contains(permanentList, prevList[i])))
			{
				...
				listBox1->Items->Add(gcnew String(prevList[i].deviceDesc.c_str()));	//äîáàâëåíèå óñá óñòðîéñòâà â ñïèñîê óñá óñòðîéñòâ
			}
		}
		if (listBox1->Items->Count>0)
			listBox1->SelectedIndex = 0;
